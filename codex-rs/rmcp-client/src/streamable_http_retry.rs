...
impl RmcpClient {
    pub(super) async fn connect_pending_transport_with_oauth_recovery(
            ...
            .await
        {
            ...
            Err(error) if Self::is_unauthorized_initialize_error(&error) => {
                ...
                // this wrapper for the retry would turn persistent 401s into a refresh loop.
    ...
    async fn connect_pending_transport_with_initialize_retries(
            ...
            .enumerate()
        {
            ...
            // A recreated `Auto` transport may resolve a different credential source. Recovery
            // must therefore refresh through the persistor belonging to the attempt that actually
            // returned 401, never the persistor captured from an earlier retryable attempt.
