...
use core_test_support::responses::without_item_metadata;
...
-> anyhow::Result<()> {
    ...
    assert_eq!(
        without_item_metadata(serde_json::Value::Array(vec![input1[2].clone()])),
    ...
    assert_eq!(
        without_item_metadata(serde_json::Value::Array(
            vec![input2[input1.len()].clone(),]
        )),
...
async fn overrides_turn_context_but_keeps_cached_prefix_and_key_constant() -> anyhow::Result<()> {
    ...
    assert_eq!(
        without_item_metadata(body2["input"].clone()),
        without_item_metadata(serde_json::Value::Array(expected_body2))
    );
...
async fn send_user_turn_with_no_changes_does_not_send_environment_context() -> anyhow::Result<()> {
    ...
    assert_eq!(
        without_item_metadata(body1["input"].clone()),
        without_item_metadata(expected_input_1)
    );
    ...
    assert_eq!(
        without_item_metadata(body2["input"].clone()),
        without_item_metadata(expected_input_2)
    );
