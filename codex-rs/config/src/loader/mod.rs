...
/// - overlay:  cloud-managed system-overlay fragments, or legacy cloud fallback
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            ...
            cloud_baseline_requirements_layers = baseline_requirements.unwrap_or_default();
            cloud_overlay_requirements_layers =
                system_overlay_requirements.unwrap_or(enterprise_managed_requirements);
            cloud_baseline_config_layers = baseline_config.unwrap_or_default();
            cloud_overlay_config_layers =
                system_overlay_config.unwrap_or(enterprise_managed_config);
        }
